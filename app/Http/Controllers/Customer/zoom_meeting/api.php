...
function createMeeting($data = array())
{
    ...
    $createMeetingArr['settings'] = array(
     ...
     $request_url = "https://api.zoom.us/v2/users/".Session::get('ZOOM_GMAIL_ID')."/meetings";
    $token = array(
        "iss" => Session::get('ZOOM_API_KEY'),
        "exp" => time() + 120 //60 seconds as suggested

    );
    $getJWTKey = JWT::encode($token, Session::get('ZOOM_API_SECRET'));
