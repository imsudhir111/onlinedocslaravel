...
class CounsellorController extends Controller
{
    ...
    public function counsellors(){
        $counsellers=Doctor::select('id','name','highest_education','experience','photo')->where(['status'=>1, 'counsellor'=>1])->get();
        return view('frontend.counsellors',compact('counsellers'));
    }
    public function counselor_detail($id){
        $counsellor_detail['doctor_info']=Doctor::select('id','name','highest_education','experience','photo','working_days')->where(['id'=>$id,'status'=>1,'counsellor'=>1])->get();
